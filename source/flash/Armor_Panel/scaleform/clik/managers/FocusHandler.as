package scaleform.clik.managers
{
    ...
    public class FocusHandler
    {
        ...
        public static function init(stage:Stage, component:UIComponent) : void
        {
            ...
            var focusHandler:scaleform.clik.managers.FocusHandler = scaleform.clik.managers.FocusHandler.getInstance();
        ...
        public function setFocus(focus:InteractiveObject, focusGroupIdx:uint = 0, mouseChange:Boolean = false) : void
        {
            ...
            if(Boolean(spr) && Boolean(mouseChange) && spr.tabEnabled == false)
            {
                focus = null;
            }
            ...
            if(actualFocus != currentFocus && (!isActualFocusTextField || Boolean(isActualFocusTextField) && !isCurrentFocusUIComponent))
            {
        ...
        protected function handleInput(event:InputEvent) : void
        {
            ...
            if(!ok || Boolean(newEvent.handled))
            {
                return;
            }
            ...
            if(actualFocus is TextField && actualFocus == focusedElement && Boolean(this.handleTextFieldInput(nav,controllerIdx)))
            {
                return;
            }
            if(actualFocus is TextField && Boolean(this.handleTextFieldInput(nav,controllerIdx)))
            {
                return;
            }
            ...
            {
                if(Boolean(stageFocusedElement) && stageFocusedElement is UIComponent)
                {
        ...
        protected function handleFocusChange(oldFocus:InteractiveObject, newFocus:InteractiveObject, event:FocusEvent) : void
        {
            ...
            if(oldFocus is TextField && newFocus == null && Boolean(CLIK.disableTextFieldToNullFocusMoves))
            {
                event.preventDefault();
                return;
            }
        ...
        protected function updateActualFocus(event:FocusEvent) : void
        {
            ...
            {
                if(!(Boolean(isActualFocusTextField) && Boolean(isCurrentFocusUIComponent)) || newFocus == null)
                {
                    if(!this.preventStageFocusChanges || Boolean(isActualFocusTextField))
                    {
                        this.setFocus(newFocus,focusGroupIdx);
                    }
                }
