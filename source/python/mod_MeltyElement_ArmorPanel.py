...
class ArmorPanel(Flash):
    ...
    def __init__(self):
        ...
        fp = open(res_modPath + "/gui/flash/ElementArmorPanelSettings.xml")
        s = fp.read()
        ...
        if config is not None:
            try:
                if common.has_key('debugRenderHitPosKey'):
                    self.toggleDrawDebugRender = keyDict[common.readString('debugRenderHitPosKey')]
                if common.has_key('posUpKey'):
                    self.hitUp = keyDict[common.readString('posUpKey')]
                if common.has_key('posDownKey'):
                    self.hitDown = keyDict[common.readString('posDownKey')]
                if common.has_key('posDefaultKey'):
                    self.hitDefault = keyDict[common.readString('posDefaultKey')]
                if common.has_key('hitHullHeight'):
                    self.defaultHitHullHeight = common.readFloat('hitHullHeight')
                if common.has_key('useTargetGunY'):
                    self.useTargetGunY = common.readBool('useTargetGunY')
                if common.has_key('gunPitchHitAngle'):
                    self.useGunPitch = common.readBool('useGunPitch')
                if common.has_key('rayLength'):
                    self.rayLength = common.readFloat('rayLength')
            except:
                print "[MeltyElement] config found but error occurred. Check values and file is encoded UTF-8 NO BOM ", res_modPath + "/gui/flash/ElementArmorPanelSettings.xml"
        else:
