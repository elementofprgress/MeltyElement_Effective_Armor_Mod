...
wotModPath = xPaths[1]
modPathFull = wotRoot + str(wotModPath)
...
class ArmorPanel(Flash):
    ...
    def __init__(self):
        ...
        self.currAimMode = "arcade"
        self.name = ""
        self.tankType = "Tank"
        self.rightArc = 0.0
        self.leftArc = 0.0
        self.stdFrontArmor = 0.0
        self.armorAngle = 0.0
        self.stdSideArmor = 0.0
        ...
        if config is not None:
            try:
                utills.eDebug("[MeltyElement] /config/ElementArmorPanelSettings.xml parsing")
                ...
                if xmlRoot.has_key('protractorOffsetWhenTD'):
                    self.effArmorOffsetY = xmlRoot.readFloat('protractorOffsetWhenTD')
            ...
            print modPathFull, "[MeltyElement] config not found"
    ...
    def stopUpdate(self):
        if self.__timeInterval is not None:
    ...
    def getPlayerTankValues(self):
        ...
        self.stdFrontArmor = hullArmor[0]
        self.stdSideArmor = hullArmor[1]
        self.armorAngle = 0
        if vTypeDesc.gun.turretYawLimits is not None:
            ...
            self.rightArc = 0.0
            self.leftArc = 0.0
        utills.eDebug("tankType:", self.tankType, "frontArmor:", self.stdFrontArmor, " sideArmor:", self.stdSideArmor, " Left:", self.leftArc, " Right:", self.rightArc)
    ...
    def setup(self, playerVehicle):
        ...
        if BigWorld.player() or hasattr(player, 'isOnArena') and player.isOnArena and playerVehicle is not None:
            ...
            print "[ELEMENT_ArmorPanel] Tank ", self.name, "type ", self.tankType, "rightArc ", self.rightArc, "leftArc ", self.leftArc, "frontArmor ", self.stdFrontArmor, "armorAngle ", self.armorAngle, "sideArmor ", self.stdSideArmor
            self.flashCall('setTankValues', [self.tankType, self.leftArc, self.rightArc, self.name, self.armorAngle])
            pass
        else:
    ...
    def AP_update(self):
        if BigWorld.player() and isAlive(BigWorld.player().playerVehicleID):  # or BigWorld.isKeyDown(Keys.KEY_LALT):
            ...
            if showArmorPanel:
                if self.currAimMode != BigWorld.player().inputHandler.ctrlModeName:
                    self.currAimMode = BigWorld.player().inputHandler.ctrlModeName
                ...
                try:
                    ...
                    utills.eDebug("Attribute Error with desiredShotPoint details are ", AttE)
                    return
                ...
                yawRads = Math.Matrix(BigWorld.player().gunRotator.turretMatrix).yaw if not self.isCamAngle else cameraTurretYaw
                yawDegrees = math.degrees(Math.Matrix(BigWorld.player().gunRotator.turretMatrix).yaw) if not self.isCamAngle else math.degrees(cameraTurretYaw)
                turretYaw = math.degrees(Math.Matrix(BigWorld.player().gunRotator.turretMatrix).yaw)
                if self.currAimMode == 'sniper' and self.tankType == 'TD' and self.isCamAngle:
                    yawDegrees = turretYaw if not math.fabs(turretYaw) > abs(self.leftArc) else yawDegrees
                turretPitch = Math.Matrix(gunRotator.gunMatrix).pitch
                if self.currAimMode != 'arcade' and self.tankType == 'SPG':
                    currArmorAngle = 0.0
                else:
                    currArmorAngle = self.getDynamicArc(ownVehicle, playerVehTypeDesc, vehicleMP, yawRads, turretPitch)
                absYawDegrees = math.fabs(math.degrees(Math.Matrix(BigWorld.player().gunRotator.turretMatrix).yaw)) if not self.isCamAngle else math.fabs(yawDegrees)
                effFrontArmor = self.getEquArmor(self.stdFrontArmor, absYawDegrees, currArmorAngle)
                effSideArmor = self.getEquArmor(self.stdSideArmor, (90 - math.fabs(absYawDegrees)), 0)
                self.flashCall('getEffArmor', [effFrontArmor, effSideArmor, yawRads, yawDegrees, turretYaw, int(self.leftArc), int(self.rightArc), self.tankType, TankYaw, self.currAimMode])
        ...
        if BigWorld.player() is not None:
            ...
            aimMode = None
            self.currAimMode = aimMode
        return

    def ArmorPanelEnableCallBack(self, AP, cid, eCA, eTD, eSPG, ordTank, ordTD, ordSPG, xSpace):
        self.isCamAngle = eCA
        self.isAP = AP
        self.TDenabled = eTD
...
armorPanel.sendFlashSettings(armorPanel.isAP, armorPanel.isCamAngle, armorPanel.TDenabled, armorPanel.SPGenabled, armorPanel.Tank_Y, armorPanel.TD_Y, armorPanel.effArmorOffsetY, armorPanel.SPG_Y, armorPanel.m2pix, armorPanel.xSpacing, 0, armorPanel.debug, armorPanel.penIndicator)
...
def ME_AP_Vehicle_onLeaveWorld(self):
    ...
    if self.isPlayerVehicle:
        ...
        if armorPanel is not None:
            armorPanel.stopUpdate()
            armorPanel.destroy()
            armorPanel = None
...
def ME_AP_FragsCollectibleStats_addVehicleStatusUpdate(self, vInfoVO):
    global armorPanel
    if (not vInfoVO.isAlive()) and (armorPanel.player_id == vInfoVO.vehicleID):
...
print "[Element_AP]: Element_ArmorPanel: Beta v0.1b  Built: 07.08.2018"
print "--------------------------------------------------------------------------------------"